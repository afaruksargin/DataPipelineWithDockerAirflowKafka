...
def write_dataframe_to_postgres(dataframe, table_name,schema_name='public'):
    ...
    try:
        ...
        connection.commit()
        print(f"DataFrame başarılı bir şekilde {table_name} tablosuna aktarıldı aktarıldı.")
...
def main():
    ...
    try :
        dataframe = consumer_func(consumer=consumer1,topics=["public"],max_messages=17000)
        if dataframe is not None:
            # BURADA DATAFRAME ÜZERİNDE İŞLEMELER GERÇEKLEŞEBİLİR.
            dataframe.columns = ["Country","Month","Year","Value"]
            yearly_dataframes = {}
            unique_years = dataframe["Year"].unique()

            for year in unique_years:
                yearly_dataframes[year] = dataframe[dataframe["Year"] == year].reset_index(drop = True)

            for year,year_df in yearly_dataframes.items():
                year_df.loc[year_df["Value"] < 1000,'Country'] = "Another"
                year_df = year_df.groupby(['Country', 'Year', 'Month']).agg({'Value': 'sum'}).reset_index()
                table_name = 'year_'+str(year)
                # 'Year' sütununu silmeden önce DataFrame'i kopyalayın
                year_df_copy = year_df.drop(columns=['Year']).copy()

                write_dataframe_to_postgres(dataframe= year_df_copy,table_name=table_name)
